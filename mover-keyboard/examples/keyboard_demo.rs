...
fn demo_basic_typing() -> Result<(), Box<dyn std::error::Error>> {
    println!("\n📝 SECTION 1: Basic Typing");
    println!("----------------------------------------");
    ...
    type_string_with_interval("Slow typing...", 0.5)?;
    sleep(Duration::from_secs(1));
...
fn demo_key_presses() -> Result<(), Box<dyn std::error::Error>> {
    println!("\n🔑 SECTION 2: Key Presses");
    println!("----------------------------------------");
...
fn demo_hotkeys() -> Result<(), Box<dyn std::error::Error>> {
    println!("\n🔥 SECTION 3: Hotkey Combinations");
    println!("----------------------------------------");
...
fn demo_advanced_patterns() -> Result<(), Box<dyn std::error::Error>> {
    ...
    type_string("Special characters: !@#$%^&*()_+-=[]{}|;':\",./<>?`~")?;
    sleep(Duration::from_millis(500));
